 ...
 */
async function generateCardsWithStream(text, deckOptions = '', textContext = '', cardType = 'basic', onProgress = null) {
  const { anthropicApiKey } = getStoredApiKeys();
  ...
  try {
    response = await fetch('/api/generate-cards-stream', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text: truncateText(text), textContext, deckOptions, cardType, userApiKey: anthropicApiKey || null })
    });
  } catch (err) {
  ...
  // If finalResult contains cards, parse using existing parser
  if (finalResult && finalResult.success && Array.isArray(finalResult.cards)) {
    return finalResult.cards.map(c => ({ front: c.front || '', back: c.back || '', deck: c.deck || 'General' }));
  }
